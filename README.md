...
| <a name="input_otel_config"></a> [otel\_config](#input\_otel\_config) | Configuration options for Opentelemetry and its resources | `map(any)` | <pre>{<br>  "otel_collector_enabled": true,<br>  "otel_operator_enabled": true<br>}</pre> | no |
...
| <a name="input_tempo_config"></a> [tempo\_config](#input\_tempo\_config) | Configuration options for Grafana Tempo | `map(any)` | <pre>{<br>  "s3_bucket_name": "grafana-tempo-skaf",<br>  "s3_bucket_region": "us-east-2",<br>  "tempo_enabled": true<br>}</pre> | no |
